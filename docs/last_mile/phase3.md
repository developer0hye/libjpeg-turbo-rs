...
**Shim fix landed alongside pattern #4 (2026-05-08).** Pattern #4 surfaced a real shim defect: `push_bytes_through_dest_mgr` in `crates/libjpeg-turbo-rs-capi/src/jpeglib.rs` ignored the boolean return of `empty_output_buffer` and unconditionally called `term_destination` even when the consumer signalled I/O suspension. A custom destination manager that returned `FALSE` (libjpeg.txt §5.5) saw the stream silently terminated and the post-suspension bytes dropped on the floor. Fix: capture the `CBoolean` return; on `FALSE`, stash the unwritten encoded tail in a new `CompressPrivate::pending_flush_tail` field, skip `term_destination`, and leave `priv_state.have_started = true` so a subsequent `jpeg_finish_compress` retry resumes the drain. The shim's deferred-encode architecture (`jpeg_write_scanlines` only buffers pixels) means upstream's per-MCU streaming suspension during `write_scanlines` is impossible by design; pattern #4 documents this divergence and pins the resume contract at the flush boundary, which is the only place suspension can fire in this shim.
...
4. ~~**P3-5** — Classic `jpeglib.h` lifecycle / custom-I/O / suspension C harness (≥ 8 tests).~~ **CLOSED 2026-05-08** — all 8 patterns active in `crates/libjpeg-turbo-rs-capi/tests/capi_classic_lifecycle.rs` (custom src/dst mgr, source suspension, destination suspension + resume + reuse-hazard guard, abort+reuse for both decompress/compress, buffered-image multi-pass, setjmp/longjmp). Pattern #4 surfaced + fixed a real shim defect: `push_bytes_through_dest_mgr` now honors `empty_output_buffer` returning `FALSE` (libjpeg.txt §5.5) — the unwritten encoded tail is stashed in `CompressPrivate::pending_flush_tail`, `term_destination` is skipped, and a follow-up `jpeg_finish_compress` resumes the drain. The earlier P3-5 pattern #8 fix (2026-05-07) wired `jpeg_read_header` to invoke `error_exit` on EOI-terminated malformed input.
