...
**Motivation.** Found 2026-07-28 by the #394 docs audit while verifying that PR's comment "a 12-bit source never converts to `Cmyk` in either arm". The 12-bit arm now returns `JpegError::Unsupported`, but its 8-bit twins panic: requesting `PixelFormat::Cmyk` for a source that is not CMYK/YCCK reaches an `unreachable!()` at three sites in `src/decode/pipeline.rs`. Measured on aarch64-darwin, 2026-07-28, via `decompress_to(&jpeg, PixelFormat::Cmyk)` on 16x8 fixtures built by our own encoder:

| Source | Result |
|---|---|
| 8-bit baseline YCbCr (4:2:0) | panic, `pipeline.rs:1411` (`grayscale/cmyk handled separately`) |
| 8-bit baseline grayscale | panic, `pipeline.rs:4422` (grayscale expansion match) |
| 8-bit lossless grayscale (SOF3) | panic, `pipeline.rs:3512` (`lossless_output_grayscale` match) |
...
## P4-72. Grayscale Colorspace Override Cannot Convert — It Only Copies a Full-Resolution Luma Plane — **OPEN**
...
**Acceptance criteria.** (1) RGB→grayscale conversion in the override path, byte-exact vs `djpeg -grayscale` on JCS_RGB streams; (2) subsampled-comp0 sources upsample component 0 and produce gray output byte-exact vs `djpeg -grayscale`; (3) the `Unsupported` guard and the JCS_RGB exclusion in `effective_output_colorspace` are deleted, their regression tests flipped to assert correct pixels; (4) the P4-21 lenient contract re-checked on these paths.

## P4-73. capi dlopen Tests Hardcode the In-Repo `target/` — Stale-Artifact False Greens Under `CARGO_TARGET_DIR` — **OPEN**
