...
**Next up**: drive P4-13 / P4-14 / P4-17 to closure as adoption pressure surfaces. Long-term backlog items live as P2-F (strided zero-copy refactor), P2-G (downstream lab uplift Qt+OpenCV), P2-H (OSS-Fuzz upstream PR + C-harness sanitizer), P2-I (remaining hard-case parity classes), and the per-ABI cdylib matrix / production packaging / no_std subset / security ops policy / cost-of-feature benchmarks — all tracked in the master plan at `/Users/yhkwon/.claude/plans/dreamy-moseying-swing.md`. None of the remaining P4-13/14/17 items block the current T3 release gate; they document divergences from upstream's contract surface that downstream production adoption will need addressed before the T4 (system v6b/v7 drop-in) work can be honestly framed.
