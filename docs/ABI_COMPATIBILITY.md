...
- **Public symbol surface: TurboJPEG 3.x + classic libjpeg API at v8 level.** Includes `tj3*` (TurboJPEG 3), a partial subset of `tj*` legacy aliases (`tjCompress2` / `tjDecompress2` only; 18 other 1.x/2.x symbols are still allowlisted-missing — see [P4-18](last_mile/phase4.md#p4-18-18-legacy-turbojpeg-1x2x-symbols-remain-allowlisted-missing--open)), and the `jpeg_*` classic API.
...
| TurboJPEG (`libturbojpeg.so.0`) | `libturbojpeg.so.0`         | `@rpath/libturbojpeg.0.dylib`    | Safe for TJ3 callers — TurboJPEG API is opaque-handle, no struct ABI. 18 legacy TJ 1.x/2.x symbols (`tjAlloc`, `tjFree`, `tjCompress`, …) remain unimplemented; see [P4-18](last_mile/phase4.md#p4-18-18-legacy-turbojpeg-1x2x-symbols-remain-allowlisted-missing--open) for the implement-vs-deprecate decision matrix. |
