...
| B9-4 / B9-5 — drop-in `libjpeg.so.62` link test against stock djpeg/cjpeg + tjunittest.c | 🔶 | B9-5 harness operational (tjunittest runs to completion on our shim after fixing `tj3Compress8` NOREALLOC in-place semantics). Stock djpeg/cjpeg/jpegtran link (B9-4) blocked on the ~33 classic `jpeg_*` symbols enumerated in `examples/stock_djpeg_cjpeg/COORDINATOR_NOTES.md` (ignored until classic API lands). |
