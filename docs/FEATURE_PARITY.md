...
- **CI on libjpeg-turbo 3.1.4.1**: apt ships 2.1.x on Ubuntu 24.04, which lacks `-lossless`, `-precision`, and SOF3 decode. CI now downloads the official `libjpeg-turbo-official_3.1.4.1_${ARCH}.deb` from upstream GitHub releases and prepends `/opt/libjpeg-turbo/bin` to `$PATH`, so test expectations match the macOS homebrew reality.
  ...
  **Currency caveat (2026-08-09).** 3.1.4.1 is the version every gate in this repository is measured against, not the current upstream release: **3.2.0 shipped 2026-06-30**. The 3.1.4.1 pin remains correct as a *behaviour-regression* baseline, but no gate here measures the 3.2 delta — which includes per-instance SIMD dispatch (upstream removed thread-local storage from its libjpeg API), RISC-V RVV kernels, 8-bit lossy → 12-bit output via `cinfo->data_precision = 12` / `tj3Decompress12()`, `jpeg_crop_scanline()` hardening, 8/16-bit PNG with ICC transfer in cjpeg/djpeg and `tj3LoadImage*`/`tj3SaveImage*`, repeated and compression-instance `tj3GetICCProfile()`, and `TJCS_DEFAULT`. Any ✅ below means parity with 3.1.4.1. Tracked as [P4-130](last_mile/phase4.md#p4-130-c-parity-oracle-is-pinned-to-3141-upstream-stable-is-320--open), which requires a two-leg oracle rather than a version bump.
