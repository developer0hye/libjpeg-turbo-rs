...
// For 8-bit lossless and 12-bit lossy precision.
// ---------------------------------------------------------------------------

/// Cross-validate 8-bit lossless and 12-bit lossy precision encoding against
/// C djpeg. For each supported precision:
/// - Rust encode -> write JPEG -> C djpeg decode -> compare pixels.
/// - 8-bit lossless must be pixel-exact (diff=0).
/// - 12-bit lossy (Q100) comparison: if djpeg outputs true 12-bit (maxval>=4095),
///   target diff=0; if djpeg outputs 8-bit, allow small rounding tolerance.
#[test]
fn c_djpeg_precision_extended_diff_zero() {
    ...
    // --- 8-bit lossless: Rust encode -> C djpeg decode -> exact match ---
    {
        let label: &str = "8bit_lossless_rust_enc_c_dec";
        ...
        let jpeg: Vec<u8> = compress_lossless(&pixels, w, h, PixelFormat::Grayscale)
            .unwrap_or_else(|e| panic!("{}: Rust lossless encode failed: {}", label, e));

        let tmp_jpg: TempFile = TempFile::new("ext_8bit_lossless.jpg");
        let tmp_pnm: TempFile = TempFile::new("ext_8bit_lossless.pgm");
        ...
        assert!(
            ...
            "{}: djpeg failed on 8-bit lossless JPEG: {}",
            label,
            String::from_utf8_lossy(&output.stderr)
        );

        let (dw, dh, _dmaxval, c_pixels) = parse_pgm_16(tmp_pnm.path());
        assert_eq!(dw, w, "{}: width mismatch", label);
        assert_eq!(dh, h, "{}: height mismatch", label);

        let expected_u16: Vec<u16> = pixels.iter().map(|&v| v as u16).collect();
        assert_eq!(
            c_pixels, expected_u16,
            "{}: 8-bit lossless Rust-encode -> C-decode must be pixel-exact",
            label
        );
    }

    // --- 12-bit lossy (Q100): Rust encode -> C djpeg decode ---
    {
        let label: &str = "12bit_lossy_rust_enc_c_dec";
        let mut pixels: Vec<i16> = Vec::with_capacity(w * h);
        for y in 0..h {
            for x in 0..w {
        ...
        let jpeg: Vec<u8> = compress_12bit(&pixels, w, h, 1, 100, Subsampling::S444)
            .unwrap_or_else(|e| panic!("{}: Rust compress_12bit failed: {}", label, e));

        let tmp_jpg: TempFile = TempFile::new("ext_12bit_lossy.jpg");
        let tmp_pnm: TempFile = TempFile::new("ext_12bit_lossy.pnm");
        std::fs::write(tmp_jpg.path(), &jpeg).expect("write temp JPEG");
        ...
        if !output.status.success() {
            ...
            if c_maxval >= 4095 {
                ...
                // djpeg scaled to 8-bit; scale Rust 12-bit values to 8-bit
                let max_diff: i32 = rust_img
                    ...
                    .max()
                    .unwrap_or(0);
                // Measured: rounding during scale + IDCT produces max_diff <= 2
                assert!(
                    max_diff <= 2,
