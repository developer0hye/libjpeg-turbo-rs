...
pub fn idct_6x6(coeffs: &[i16; 64], quant: &[u16; 64], output: &mut [u8; 36]) {
    ...
    for col in 0..6 {
        ...
        let tmp10 = tmp1 + tmp0m;
        let tmp12 = tmp1 - tmp0m;
        ...
        ws[6 + col] = tmp11 + tmp1f;
        ws[24 + col] = tmp11 - tmp1f;
...
pub fn idct_7x7(coeffs: &[i16; 64], quant: &[u16; 64], output: &mut [u8; 49]) {
    ...
    for col in 0..7 {
        ...
        let tmp0 = tmp1 - tmp2i;
        let tmp1 = tmp1 + tmp2i;
        let tmp2 = (z2 + z3) * (-fix(1.378756276));
        let tmp1 = tmp1 + tmp2;
...
pub fn idct_9x9(coeffs: &[i16; 64], quant: &[u16; 64], output: &mut [u8; 81]) {
    ...
    for col in 0..8 {
        ...
        let tmp1 = tmp0 + tmp3;
        let tmp2 = tmp0 - tmp3 - tmp3;
        let tmp0 = (z1 - z2) * fix(0.707106781);
        let tmp11 = tmp2 + tmp0;
        let tmp14 = tmp2 - tmp0 - tmp0;
        let tmp0 = (z1 + z2) * fix(1.328926049);
        let tmp2 = z1 * fix(1.083350441);
        let tmp3 = z2 * fix(0.245575608);
        let tmp10 = tmp1 + tmp0 - tmp3;
        let tmp12 = tmp1 - tmp0 + tmp2;
        let tmp13 = tmp1 - tmp2 + tmp3;
        let (z1, z2, z3, z4) = (c(1), c(3), c(5), c(7));
        let z2 = z2 * (-fix(1.224744871));
        let tmp2 = (z1 + z3) * fix(0.909038955);
        let tmp3 = (z1 + z4) * fix(0.483689525);
        let tmp0 = tmp2 + tmp3 - z2;
        let tmp1 = (z3 - z4) * fix(1.392728481);
        let tmp2 = tmp2 + z2 - tmp1;
...
pub fn idct_10x10(coeffs: &[i16; 64], quant: &[u16; 64], output: &mut [u8; 100]) {
    ...
    for col in 0..8 {
        ...
        let tmp12 = z1 + z2 * fix(0.513743148);
        let tmp13 = z1 - z3 * fix(2.176250899);
        let tmp20 = tmp10 + tmp12;
        let tmp24 = tmp10 - tmp12;
        let tmp21 = tmp11 + tmp13;
        let tmp23 = tmp11 - tmp13;
        let (z1, z2, z3, z4) = (c(1), c(3), c(5), c(7));
        let tmp11 = z2 + z4;
        let tmp13 = z2 - z4;
        let tmp12 = tmp13 * fix(0.309016994);
        let z5 = z3 << CONST_BITS;
        let z2 = tmp11 * fix(0.951056516);
        let z4 = z5 + tmp12;
        ...
        ws[16 + col] = tmp22 + tmp12;
        ws[56 + col] = tmp22 - tmp12;
...
pub fn idct_11x11(coeffs: &[i16; 64], quant: &[u16; 64], output: &mut [u8; 121]) {
    ...
    for col in 0..8 {
        ...
        let tmp11 = tmp11 + z1t;
        let tmp13 = tmp13 + z1t + z4 * fix(2.102458632);
        let tmp14 =
...
pub fn idct_12x12(coeffs: &[i16; 64], quant: &[u16; 64], output: &mut [u8; 144]) {
    ...
    for col in 0..8 {
        ...
        let tmp11 = z3 - z4;
        let z1 = c(2);
        let z4 = z1 * fix(1.366025404);
        let z1s = z1 << CONST_BITS;
        let z2 = c(6);
        let z2s = z2 << CONST_BITS;
        let tmp12 = z1s - z2s;
        let tmp21 = z3 + tmp12;
        let tmp24 = z3 - tmp12;
        let tmp12 = z4 + z2s;
        let tmp20 = tmp10 + tmp12;
        let tmp25 = tmp10 - tmp12;
        let tmp12 = z4 - z1s - z2s;
        let tmp22 = tmp11 + tmp12;
        let tmp23 = tmp11 - tmp12;
        let (z1, z2, z3, z4) = (c(1), c(3), c(5), c(7));
        let tmp11 = z2 * fix(1.306562965);
        let tmp14 = z2 * (-FIX_0_541196100);
        let tmp10 = z1 + z3;
        let tmp15 = (tmp10 + z4) * fix(0.860918669);
...
pub fn idct_13x13(coeffs: &[i16; 64], quant: &[u16; 64], output: &mut [u8; 169]) {
    ...
    for col in 0..8 {
        ...
        let tmp10 = z3 + z4;
        let tmp11 = z3 - z4;
        let tmp12 = tmp10 * fix(1.155388986);
        let tmp13 = tmp11 * fix(0.096834934) + z1;
        let tmp20 = z2 * fix(1.373119086) + tmp12 + tmp13;
        let tmp22 = z2 * fix(0.501487041) - tmp12 + tmp13;
        let tmp12 = tmp10 * fix(0.316450131);
        let tmp13 = tmp11 * fix(0.486914739) + z1;
        let tmp21 = z2 * fix(1.058554052) - tmp12 + tmp13;
        let tmp25 = z2 * (-fix(1.252223920)) + tmp12 + tmp13;
        let tmp12 = tmp10 * fix(0.435816023);
        let tmp13 = tmp11 * fix(0.937303064) - z1;
        let tmp23 = z2 * (-fix(0.170464608)) - tmp12 - tmp13;
        let tmp24 = z2 * (-fix(0.803364869)) + tmp12 - tmp13;
        let tmp26 = (tmp11 - z2) * fix(1.414213562) + z1;
        let (z1, z2, z3, z4) = (c(1), c(3), c(5), c(7));
        let tmp11 = (z1 + z2) * fix(1.322312651);
        let tmp12 = (z1 + z3) * fix(1.163874945);
        let tmp15 = z1 + z4;
        let tmp13 = tmp15 * fix(0.937797057);
        ...
        let tmp11 = tmp11 + tmp14;
        let tmp13 = tmp13 + tmp14 + z4 * fix(2.205608352);
        let tmp14 = (z3 + z4) * (-fix(0.657217813));
        let tmp12 = tmp12 + tmp14;
        let tmp13 = tmp13 + tmp14;
        let tmp15 = tmp15 * fix(0.338443458);
        let tmp14 = tmp15 + z1 * fix(0.318774355) - z2 * fix(0.466105296);
        let z1t = (z3 - z2) * fix(0.937797057);
        let tmp14 = tmp14 + z1t;
...
pub fn idct_14x14(coeffs: &[i16; 64], quant: &[u16; 64], output: &mut [u8; 196]) {
    ...
    for col in 0..8 {
        ...
        let tmp13 = z3 + z1 * fix(0.273079590);
        let tmp14 = z3 - z2 * fix(1.719280954);
        let tmp15 = z1 * fix(0.613604268) - z2 * fix(1.378756276);
        let tmp20 = tmp10 + tmp13;
        let tmp26 = tmp10 - tmp13;
        let tmp21 = tmp11 + tmp14;
        let tmp25 = tmp11 - tmp14;
        let tmp22 = tmp12 + tmp15;
        let tmp24 = tmp12 - tmp15;
        let (z1, z2, z3, z4) = (c(1), c(3), c(5), c(7));
        let tmp13 = z4 << CONST_BITS;
        let tmp14 = z1 + z3;
        let tmp11 = (z1 + z2) * fix(1.334852607);
        let tmp12 = tmp14 * fix(1.197448846);
        let tmp10 = tmp11 + tmp12 + tmp13 - z1 * fix(1.126980169);
        let tmp14 = tmp14 * fix(0.752406978);
        let tmp16 = tmp14 - z1 * fix(1.061150426);
        let z1 = z1 - z2;
        let tmp15 = z1 * fix(0.467085129) - tmp13;
        let tmp16 = tmp16 + tmp15;
        let z1 = z1 + z4;
        ...
        ws[24 + col] = tmp23 + tmp13;
        ws[80 + col] = tmp23 - tmp13;
        ws[32 + col] = (tmp24 + tmp14) >> (CONST_BITS - PASS1_BITS);
    ...
    for row in 0..14 {
        ...
        let tmp16 = tmp14 - z1 * fix(1.061150426);
        let z1 = z1 - z2;
        let tmp15 = z1 * fix(0.467085129) - z4s;
        let tmp16 = tmp16 + tmp15;
...
pub fn idct_15x15(coeffs: &[i16; 64], quant: &[u16; 64], output: &mut [u8; 225]) {
    ...
    for col in 0..8 {
        ...
        let z4 = z2 - z3;
        let z3 = z3 + z2;
        let tmp10 = z3 * fix(1.337628990);
        let tmp11 = z4 * fix(0.045680613);
        let z2 = z2 * fix(1.439773946);
        let tmp20 = tmp13 + tmp10 + tmp11;
        let tmp23 = tmp12 - tmp10 + tmp11 + z2;
        let tmp10 = z3 * fix(0.547059574);
        let tmp11 = z4 * fix(0.399234004);
        let tmp25 = tmp13 - tmp10 - tmp11;
        let tmp26 = tmp12 + tmp10 - tmp11 - z2;
        let tmp10 = z3 * fix(0.790569415);
        let tmp11 = z4 * fix(0.353553391);
        let tmp21 = tmp12 + tmp10 + tmp11;
        let tmp24 = tmp13 - tmp10 + tmp11;
        let tmp11 = tmp11 + tmp11;
        let tmp22 = z1 + tmp11;
        let tmp27 = z1 - tmp11 - tmp11;
        let (z1, z2) = (c(1), c(3));
        let z4 = c(5);
        let z3 = z4 * fix(1.224744871);
        let z4 = c(7);
        let tmp13 = z2 - z4;
        let tmp15 = (z1 + tmp13) * fix(0.831253876);
        let tmp11 = tmp15 + z1 * fix(0.513743148);
        let tmp14 = tmp15 - tmp13 * fix(2.176250899);
        let tmp13 = z2 * (-fix(0.831253876));
        let tmp15 = z2 * (-fix(1.344997024));
        let z2 = z1 - z4;
        let tmp12 = z3 + z2 * fix(1.406466353);
...
pub fn idct_16x16(coeffs: &[i16; 64], quant: &[u16; 64], output: &mut [u8; 256]) {
    ...
    for col in 0..8 {
        ...
        let (z1, z2) = (c(2), c(6));
        let z3 = z1 - z2;
        let z4 = z3 * fix(0.275899379);
        let z3 = z3 * fix(1.387039845);
        let tmp0 = z3 + z2 * FIX_2_562915447;
        let tmp1 = z4 + z1 * FIX_0_899976223;
        let tmp2 = z3 - z1 * fix(0.601344887);
        let tmp3 = z4 - z2 * fix(0.509795579);
        ...
        let (z1, z2, z3, z4) = (c(1), c(3), c(5), c(7));
        let tmp11 = z1 + z3;
        let tmp1 = (z1 + z2) * fix(1.353318001);
        let tmp2 = tmp11 * fix(1.247225013);
        let tmp3 = (z1 + z4) * fix(1.093201867);
        let tmp10 = (z1 - z4) * fix(0.897167586);
        let tmp11 = tmp11 * fix(0.666655658);
        let tmp12 = (z1 - z2) * fix(0.410524528);
        let tmp0 = tmp1 + tmp2 + tmp3 - z1 * fix(2.286341144);
        let tmp13 = tmp10 + tmp11 + tmp12 - z1 * fix(1.835730603);
        let z1t = (z2 + z3) * fix(0.138617169);
        let tmp1 = tmp1 + z1t + z2 * fix(0.071888074);
        let tmp2 = tmp2 + z1t - z3 * fix(1.125726048);
        let z1t = (z3 - z2) * fix(1.407403738);
        let tmp11 = tmp11 + z1t - z3 * fix(0.766367282);
        let tmp12 = tmp12 + z1t + z2 * fix(1.971951411);
        let z2 = z2 + z4;
        let z1t = z2 * (-fix(0.666655658));
        let tmp1 = tmp1 + z1t;
        let tmp3 = tmp3 + z1t + z4 * fix(1.065388962);
        let z2 = z2 * (-fix(1.247225013));
        let tmp10 = tmp10 + z2 + z4 * fix(3.141271809);
        let tmp12 = tmp12 + z2;
        let z2 = (z3 + z4) * (-fix(1.353318001));
        let tmp2 = tmp2 + z2;
        let tmp3 = tmp3 + z2;
        let z2 = (z4 - z3) * fix(0.410524528);
        let tmp10 = tmp10 + z2;
        let tmp11 = tmp11 + z2;
