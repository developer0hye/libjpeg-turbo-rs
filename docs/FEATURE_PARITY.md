...
- Treat the checklist above as the source of truth. Open follow-ups (tracked in `docs/last_mile/phase4.md`): P4-4 (panic guard on every C-ABI entry point — PARTIAL through 2026-05-17, jpeglib.rs sweep in progress); P4-5 (classic state-machine pathological coverage — PARTIAL, 1/3 patterns wired); P4-9 (strided / zero-copy direct path for multimedia pipelines); P4-10 (multi-distro downstream compatibility lab uplift); P4-12 (encoder/decoder hard-case parity corpus). PNG for `tj3LoadImage8()` / `tj3SaveImage8()` is gated by the optional `png` feature flag, mirroring upstream's `PNG_SUPPORTED` build-time flag.
