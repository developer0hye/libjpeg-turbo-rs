...
**Status (2026-07-30): closed.** The function now rejects `dc_tables.len() < nc` with a typed `JpegError::CorruptData` before decoding, mirroring the guard the 8-bit twin `decode/pipeline.rs::decode_lossless_huffman` already carried (added from an earlier `fuzz_decompress_lenient` finding — the arbitrary-precision path simply never received it). Pinned by `tests/regression_lossless_partial_scan.rs`: the reported 3/2 shape, every `Ns < Nf` combination for `Nf` in 2..=4 (so the fix cannot be a special case), and an `Ns == Nf` control that still decodes to a uniform plane for each component count.
