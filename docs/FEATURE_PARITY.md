...
- **`unsafe` gates** (P4-141): `tests/parser_unsafe_gate.rs` holds twenty-six parser / control-plane sources and five scalar kernels at zero `unsafe` (criterion 5), and `tests/unsafe_inventory_gate.rs` diffs `docs/UNSAFE_INVENTORY.md` — every root-crate `unsafe` site keyed by file and enclosing item, with invariant, reachability, regression test, tool coverage and reviewer — against the tree, failing on any added, removed or moved site (criterion 4, root crate; the C-ABI crate is not yet inventoried). Both share the lexer in `tests/helpers/unsafe_scan.rs`.
