...
/// Progressive + arithmetic + row-mode restart now produces a valid
/// SOF10 stream with DRI and RST markers. Mirrors the baseline
/// arithmetic restart test above but adds `progressive=1`, exercising
/// the per-scan DRI accounting + per-MCU/-block restart counter
/// added to `write_coefficients_progressive_arithmetic` (commit
/// closing G2 in `docs/LAST_MILE.md`).
#[test]
fn write_coefficients_progressive_arithmetic_with_row_restart_emits_rst_and_dri() {
    let lib = unsafe { libloading::Library::new(cdylib_path()) }.expect("dlopen");
    let (jpeg_in, _src, _w, _h_px) = build_fixture_jpeg(&lib);
    ...
    assert!(
        !transcoded.is_empty(),
        "progressive arithmetic + restart must produce non-empty output"
    );
    assert_eq!(&transcoded[..2], &[0xFF, 0xD8], "missing SOI");
    assert_eq!(
        &transcoded[transcoded.len() - 2..],
    ...
    assert!(
        has_sof10,
        "progressive arithmetic stream must contain SOF10 (FF CA)"
    );
    let has_dri: bool = transcoded.windows(2).any(|w| w[0] == 0xFF && w[1] == 0xDD);
    assert!(
        has_dri,
        "row-mode restart must emit DRI segment in progressive arithmetic stream"
    );
    let has_rst: bool = transcoded
        .windows(2)
        .any(|w| w[0] == 0xFF && (0xD0..=0xD7).contains(&w[1]));
    assert!(
        has_rst,
        "progressive arithmetic stream must contain RST markers when restart is set"
    );
}

/// Round-trip: encode a fixture, transcode through
/// `jpeg_write_coefficients` with progressive + arithmetic + row-mode
/// restart, decode the result, and confirm the pixels match the
/// straight (non-restart) decode of the same fixture. Catches any
/// regression in the entropy stream itself, not just marker presence.
#[test]
fn write_coefficients_progressive_arithmetic_restart_roundtrip_pixel_exact() {
    let lib = unsafe { libloading::Library::new(cdylib_path()) }.expect("dlopen");
    let (jpeg_in, _src_pixels, src_w, src_h) = build_fixture_jpeg(&lib);

    // Decode the source for ground-truth pixels (without restart).
    let (gt_pixels, gt_w, gt_h) = decode_jpeg(&lib, &jpeg_in);
    assert_eq!(gt_w, src_w);
    assert_eq!(gt_h, src_h);
    ...
    let (round_pixels, round_w, round_h) = decode_jpeg(&lib, &transcoded);
    assert_eq!(round_w, gt_w);
    assert_eq!(round_h, gt_h);
    assert_eq!(
        round_pixels.len(),
        gt_pixels.len(),
        "decoded pixel buffer length must match"
    );
    // Coefficient transcoding is mathematically lossless — the decoded
    // pixels must be identical to the ground truth, even with restart
    // markers re-encoded mid-stream.
    assert_eq!(
        round_pixels, gt_pixels,
        "progressive arithmetic + restart transcode must round-trip pixel-exact"
    );
}
