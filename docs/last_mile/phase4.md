...
**Root cause (corrected 2026-07-25).** The original "later encoder change" hypothesis was wrong. `git bisect` plus per-commit generator runs showed the seeds' committing commit `a9dfca0` **itself** generates today's (922-byte) bytes — the committed 830-byte seeds never matched any committed encoder; they were generated from a stale/dirty working tree before commit. No encoder behavior ever changed for this shape. Both byte variants decode pixel-identically under `djpeg` (benign alternate encoding), so no decoder/encoder action was needed.

**Resolution.** (1) Regenerated seeds committed — a clean checkout + full test run now leaves the tree clean. (2) Drift guard added: `fan_out_write` records every overwrite-with-different-bytes into `DRIFTED_SEEDS`, and `generate_seeds` asserts the list is empty, printing the offending paths — a generator/encoder change that alters seed bytes now fails the test (and CI) in the PR that introduces it, with the regenerated files already on disk ready to review + commit. Brand-new (previously absent) files are deliberately not flagged so fresh checkouts without the corpus still generate cleanly.

**Status (2026-07-25): closed.** Proof: `cargo test --release --test generate_fuzz_seeds` fails with "14 committed fuzz seed(s) drifted" before the seed re-commit and passes after; old vs new seed decode compared byte-equal via `djpeg -ppm` + `cmp`.
...
29. ~~**P4-33** — verify the encoder change behind the 8×64/64×8 4:2:0 output drift, re-commit the regenerated `aspect_*` seeds, and add a drift guard so `cargo test` on a clean checkout leaves the tree clean.~~ **CLOSED 2026-07-25** — no encoder change existed: bisect proved the seeds were stale at their committing commit (`a9dfca0` already generated today's bytes). Seeds re-committed; `generate_seeds` now asserts zero drifted overwrites.
