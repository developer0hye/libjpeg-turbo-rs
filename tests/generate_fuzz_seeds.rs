...
/// P4-33: the committed `aspect_*` seeds never matched any committed
/// encoder (they were generated from a stale working tree and silently
/// rewritten by every full test run); the test now fails on drift (see
/// the assertion in `generate_seeds`) so a mismatch between generator
/// output and the committed corpus must be resolved in the PR that
/// introduces it.
///
...
/// Overwrites of existing files with different content are recorded in
/// [`DRIFTED_SEEDS`]; brand-new files are not (a fresh checkout without
/// the corpus must still generate cleanly).
