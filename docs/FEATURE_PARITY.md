...
- **B9-2 Pillow / B9-3 ImageMagick / B9-4 stock djpeg–cjpeg–jpegtran**: link harnesses landed (`examples/pillow_smoke/`, `examples/imagemagick_smoke/`, `examples/stock_djpeg_cjpeg/`, `tests/capi_*_compat.rs`, `tests/capi_stock_tool_link.rs`). All three are blocked behind the classic `jpeg_*` API (~33 symbols enumerated in `examples/stock_djpeg_cjpeg/COORDINATOR_NOTES.md`). Tests are `#[ignore]`d with a precise blocker note so they auto-activate the day A1-12 lands.
