...
- **Which upstream release each C oracle runs against (P4-130)**: apt ships 2.1.x on Ubuntu 24.04, which lacks `-lossless`, `-precision`, and SOF3 decode, so CI installs upstream's own `libjpeg-turbo-official_${VERSION}_${ARCH}.deb` under `/opt/libjpeg-turbo` instead. Since 2026-08-17 there are **two tool legs, not one**, and `docs/oracle_versions.tsv` is the machine-checked statement of what each one is:
  ...
  | --- | --- | --- |
  | `tool-baseline` | **3.1.4.1** (2026-03-27) | The behaviour-regression leg: `Integration Tests`, the `Full C Parity` exhaustive matrices on both architectures, the three `Cross-Arch SIMD` backend legs, `fuzz-smoke.yml`'s three differential targets, `ci.yml`'s `mutants-in-diff` (whose oracle decides whether a mutant reads as caught), and `ci.yml`'s `test-cross-encode`, a source build. `ci.yml`'s `test-corpus` builds it from source too, and since 2026-09-07 both it and `test-cross-encode` are *paired* legs rather than single ones. The release the expectations in this document were written against. Since 2026-08-18 the aarch64 `Full C Parity` legs and `test-cross-encode` build it from source rather than taking `brew install jpeg-turbo`, which had no per-version formula and would have re-baselined those matrices silently the week homebrew moved. Every job that provisions this role now *asserts* the release it installed, at the path it installed it to, rather than printing `djpeg -version` into a log nobody reads. |
  | `tool-current` | **3.2.0** (2026-06-30) | The current-parity leg: `Integration Tests (oracle 3.2.0)` runs the root `cargo test --tests` matrix *and* the C-ABI crate's TurboJPEG and stock-tool oracle suites against current upstream stable, and since 2026-08-18 `Full C Parity (x86_64, oracle 3.2.0)` / `Full C Parity (aarch64, oracle 3.2.0)` run the exhaustive matrices there too. The same day the three **cross-arch SIMD** legs were paired — `Test (linux-aarch64 NEON, oracle 3.2.0)`, `Test (linux-x86_64 AVX2, oracle 3.2.0)` and `Build (linux-x86_64, AVX2 disabled at compile time, oracle 3.2.0)` — each running the root matrix on the backend its baseline twin runs it on. Since 2026-09-07 `Corpus Test (C parity, oracle 3.2.0)` runs the real-world corpus comparison there as well — 9,216 files regenerated by *this* leg's `cjpeg` plus the 524 committed fuzz seeds and fixtures, 9,740 in all, every one decoded, re-encoded and transformed against its `djpeg`/`cjpeg`/`jpegtran`. The same day `C Interop (macos-latest, oracle 3.2.0)` paired `ci.yml`'s `test-cross-encode`: the whole root `cargo test --tests` matrix on macOS aarch64 — on every pull request, where it had been measured against 3.2.0 only weekly by the four selected `Full C Parity` matrices — against a 3.2.0 source build at `/tmp/ljt320/prefix`, since upstream ships no macOS package. The install is selected by `LIBJPEG_TURBO_PREFIX` rather than PATH order, and on the corpus leg by that variable alone: nothing is added to PATH, so a harness that stopped reading it would find no tool and the leg would fail rather than measure something else. Measured green at 223 suite sections / 2351 tests when the leg landed, plus 14 sections / 78 tests when the C-ABI suites joined it, plus the exhaustive matrices, measured unfiltered at 15 tests whose printed tallies sum to 19,439 compared cases — a CI leg runs 13 of them, since the `c_tjdecomptest` step keeps its `c_tjdecomptest_full` filter (macOS aarch64, all three). |
  ...
  | `submodule` | **3.2.0** (2026-06-30) — 3.1.90 = 3.2 beta1 until 2026-09-07 | `references/libjpeg-turbo`: built with `WITH_JPEG8=1` as the *baseline* classic-ABI trace oracle (`/tmp/ljt8/prefix`), and the source every `j*.c:NNN` citation in this repository quotes. Bumped with every citation remapped against the tag's diff; it stays a separate row from `trace-current` because the two move for different reasons. |

  So a ✅ backed by a *tool* oracle means "matches 3.1.4.1 **and** 3.2.0" — for the root differential matrix since 2026-08-17, for the six C-ABI suites that take the deb since the C-ABI half of criterion 1 landed, and since 2026-08-18 for the four exhaustive `full-c-parity` matrices on x86_64 *and* aarch64 and for the three cross-arch backends (Linux NEON, AVX2-enabled codegen, and codegen with AVX2/SSE4.2 disabled), all of which are separate measurements because each compares one SIMD backend's output against C's — and since 2026-09-07 for the root matrix on macOS aarch64 on every pull request, where `C Interop (macos-latest, oracle 3.2.0)` pairs the one leg that runs the whole suite there. The eight **classic-ABI trace** suites pair differently and the difference matters: their baseline oracle is `/tmp/ljt8/prefix`, built from the submodule, so a ✅ there meant "matches **3.1.90 and 3.2.0**" until 2026-09-07 and since then means "matches 3.2.0 at two builds" — the submodule's and the tagged clone's — until upstream ships again; it has never involved 3.1.4.1. That is not an oversight to tidy away — those suites compare compiled traces at the v8 ABI, which the 3.1.4.1 deb cannot provide at all. A ✅ backed by a *source citation* meant 3.1.90 until 2026-09-07 and means 3.2.0 since. That split existed before it was written down — it is what the P4-130 triage found first. `tests/oracle_version_pins.rs` fails if a workflow pins a version this table does not declare, if it declares a leg no workflow installs, if a C-ABI suite or an exhaustive matrix that compares against C runs on the baseline leg alone, or if a `Full C Parity` leg installs a release the manifest does not name or does not name the prefix it measures. Since 2026-08-18 it also reads **every job in every workflow** rather than a list of workflow files, and fails one that installs a C libjpeg-turbo under a package manager's name for it, that never asserts the release it installed, or that checks one prefix and resolves its C tools from another — on a macOS runner, where `helpers::c_tool_path` reads `/opt/homebrew/bin` before PATH, only `LIBJPEG_TURBO_PREFIX` counts as selecting one. *Pairing* is read from the jobs the same way: an oracle-installing job must be a baseline with a `-current-oracle` twin, that twin, or a row in the gate's `UNPAIRED_ORACLE_JOBS` inventory carrying the reason it still answers at one release. Two jobs are on that inventory today: `mutants-in-diff` and `fuzz-smoke.yml`'s matrix. Every discovered pair is then compared by one of two rules and by exactly one — a leg naming oracle-backed suites by their *selection*, a leg naming none by the commands it runs — every cargo subcommand that can reach an oracle, since `test-corpus` measures with `cargo run --release --example corpus_test` and a comparison reading `cargo test` alone saw none of that leg — the runner it runs on, and each test step's *effective* environment (job-level overlaid by step-level, minus the oracle prefix), so a twin that drops `RUSTFLAGS` or overrides it on one step compiles a different backend and fails rather than passing. A suite neither leg names is credited when the pair runs one identical whole-root-crate command, since `cargo test --tests` executes it at both releases; what such a run does not cover is a selection widening past the default set, which in this repository means the serial step's `--include-ignored` timing assertions.
...
- **`unsafe` gates** (P4-141): `tests/parser_unsafe_gate.rs` holds twenty-six parser / control-plane sources and five scalar kernels at zero `unsafe` (criterion 5), and `tests/unsafe_inventory_gate.rs` diffs `docs/UNSAFE_INVENTORY.md` and `docs/UNSAFE_INVENTORY_CAPI.md` — every root-crate and C-ABI-crate `unsafe` site keyed by file and enclosing item, with invariant, reachability, regression test, tool coverage and reviewer — against their trees, failing on any added, removed or moved site (criterion 4, closed 2026-09-09 when `jpeglib.rs`'s 501 sites landed and the gate's `DEFERRED` list emptied). Both share the lexer in `tests/helpers/unsafe_scan.rs`.
- **Miri coverage suites and their gate** (P4-141 criterion 1, 2026-09-09): `tests/miri_public_api.rs` (progressive output and the `BitWriter` arena through the public API), `tests/miri_alloc_failure.rs` (a `#[global_allocator]` that refuses one exact size on the calling thread, with a committed selftest that it fires) and `tests/miri_once_init.rs` (six threads racing the standard-table once-cell under `-Zmiri-many-seeds`), plus the doctests, are all selected by the `miri` job. `tests/miri_coverage_gate.rs` reads that job out of `ci.yml` and fails when a step stops naming a suite, drops `--doc` or `-Zmiri-many-seeds`, becomes non-executing (`--no-run`, `--list`, `if:`, `continue-on-error:`), narrows a suite with a libtest filter, or when a suite grows a `cfg` on `miri`.
