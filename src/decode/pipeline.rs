...
impl<'a> Decoder<'a> {
    ...
    #[allow(clippy::too_many_arguments)]
    fn upsample_component_plane<'plane>(
        &self,
...
// ---------------------------------------------------------------------------
// P4-58 (issue #357): windowed interleaved-baseline decode.
//
// The incremental reader (`crate::api::incremental`) feeds a sliding
// window of entropy bytes; each `baseline_stream_step` call decodes as
// many whole MCU rows as the window allows, checkpointing at row
// starts. A row whose decode runs off the window end is rolled back
// (the checkpoint is simply not advanced — the deterministic IDCT
// writes are re-done identically on retry) and the caller extends the
// window. The fast slice path above is untouched: it constructs its
// `BitReader` with `is_final = true`, where starvation can never fire.
// ---------------------------------------------------------------------------

/// Cross-call state for a windowed baseline decode: geometry computed
/// once, plus the row cursor and the entropy checkpoint.
pub(crate) struct BaselineStreamState {
    mcus_x: usize,
    mcus_y: usize,
    comp_block_sizes: [usize; 4],
    /// Next MCU row to decode (rows below are committed).
    mcu_y: usize,
    mcu_count: u32,
    /// Entropy cursor at the start of row `mcu_y`, relative to the
    /// window the caller passes (the caller keeps window offsets and
    /// checkpoint in lock-step via [`Self::rebase`]).
