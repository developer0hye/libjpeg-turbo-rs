...
The table below is the live OPEN set across all filing dates. **P4-13 / P4-14 / P4-17** were filed 2026-05-18 after an independent cold review (Claude + codex, two-pass) surfaced four C-ABI fidelity gaps the prior Phase-4 closures missed and a falsifying flaw in the P4-5 closure's primary suspension test. **P4-20 / P4-21** followed from the IDCT i16-overflow (P4-19) and non-standard-sampling work. **P4-23** was filed 2026-05-30 from a local 100,000-iteration `fuzz_decode_diff_c` smoke sweep (seed 424242, `~/smoke100k/`); its sibling **P4-22** from the same sweep was fixed and closed 2026-05-31. **P4-24** was filed 2026-05-31 from the P4-22 code review (an arithmetic-path sibling of P4-22):
...
| P4-23 | [lenient mode rejects corrupt baseline entropy ("invalid Huffman code") djpeg silently conceals](last_mile/phase4.md#p4-23-lenient-mode-rejects-corrupt-baseline-entropy-data-invalid-huffman-code-that-djpeg-silently-conceals--open) | 4 | OPEN | djpeg emits a 65×65 raster (exit 0, no stderr); Rust lenient `Err(CorruptData("invalid Huffman code"))`. Real `(Some,Rejected)` drop-in-regression panic. Repro: `cargo test --test cross_check_fuzz_decode_diff_c_multiscan -- --ignored corrupt_huffman_65x65_422_lenient_matches_djpeg` (fixture `tests/fixtures/fuzz_repro/corrupt_huffman_65x65_422.jpg`). |
| P4-24 | [arithmetic sequential (SOF9) non-interleaved multi-scan decodes only the first scan, wrong plane fill](last_mile/phase4.md#p4-24-arithmetic-sequential-sof9-non-interleaved-multi-scan-decodes-only-the-first-scan-with-wrong-plane-fill--open) | 4 | OPEN | Sibling of P4-22 in `decode_arithmetic_planes`: no multi-scan dispatch → drops scans + 0-fill (not 128) + `unwrap_or(0)` `Cs`-misroute. NOT reachable via `fuzz_decode_diff_c` (arithmetic is skipped); real arithmetic multi-scan inputs only. Found in the P4-22 review. |
...
**Next up**: **P4-23** is the highest-value open item — a confirmed latent `fuzz_decode_diff_c` panic class (lenient-mode reject of corrupt entropy djpeg silently conceals), to close before the next fuzz-corpus expansion. (Its sibling **P4-22**, a multi-scan non-interleaved baseline pixel divergence, was fixed + closed 2026-05-31 — never-scanned component planes are now pre-filled with the level-shift midpoint 128.) **P4-20 / P4-21 / P4-24** are lower-urgency decoder divergences reachable only on corrupt/non-standard inputs (SSE2-only IDCT i16-overflow, AVX2 unaffected; chroma-out-samples-luma reject; arithmetic non-interleaved multi-scan, not `fuzz_decode_diff_c`-reachable). **P4-13 / P4-14 / P4-17** are C-ABI contract gaps to drive to closure as adoption pressure surfaces. Long-term backlog items live as P2-F (strided zero-copy refactor), P2-G (downstream lab uplift Qt+OpenCV), P2-H (OSS-Fuzz upstream PR + C-harness sanitizer), P2-I (remaining hard-case parity classes), and the per-ABI cdylib matrix / production packaging / no_std subset / security ops policy / cost-of-feature benchmarks — all tracked in the master plan at `/Users/yhkwon/.claude/plans/dreamy-moseying-swing.md`. None of these block the current T3 release gate: P4-13/14/17 document upstream contract-surface divergences for T4 framing, and P4-20/21/23/24 only trigger on corrupt or pathologically-crafted inputs.
