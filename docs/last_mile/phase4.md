...
## P4-58. Rust-Native Incremental Decode Source (Bounded Input Memory) — **OPEN**
...
## P4-63. zune-jpeg Adoption-Gap Program (#380–#392) — **OPEN**
...
**Motivation.** Filed 2026-07-28 from #386's first fully clean worktree run. ~16 capi test files (`format_message.rs` `cdylib_path()`, `tjunittest_link.rs`, `libtiff_integration.rs`, `capi_jpeg_read_header_tables_only.rs`, the install/pillow harnesses, …) resolve the release cdylib as `<workspace_root>/target/release/liblibjpeg_turbo_rs_capi.*`, while the `cargo build -p libjpeg-turbo-rs-capi --release` they spawn inherits the caller's `CARGO_TARGET_DIR`. Consequences on a host that offloads the target dir (this repo's standard setup): a fresh checkout FAILS all 16 (build lands elsewhere, probe misses), and a checkout with an old in-repo `target/release/` silently dlopens the **stale** shim — the main checkout had exactly that, so weeks of local capi "greens" validated an outdated artifact. CI is unaffected (no `CARGO_TARGET_DIR` override). `examples/pillow_smoke/run.sh` was fixed in the #386 branch as the pattern to follow.

**Acceptance criteria.** A shared helper (e.g. `tests/helpers`) resolves the cdylib honoring `CARGO_TARGET_DIR` (absolute or repo-relative), used by every dlopen-class test; each test dlopens an artifact at least as new as the current `src/` mtime or rebuilds unconditionally; a worktree with `CARGO_TARGET_DIR` set and no in-repo `target/` passes the full capi suite.
