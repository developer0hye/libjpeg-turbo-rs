...
**Next up**: the 100k smoke sweep's two CI-reachable bugs — **P4-22** (multi-scan non-interleaved baseline pixel divergence; never-scanned planes now pre-filled 128) and **P4-23** (lenient recovery for corrupt entropy djpeg silently conceals) — are both fixed + closed 2026-05-31. The P4-22 arithmetic sibling **P4-24** is also closed (2026-06-01). The remaining decoder items **P4-20 / P4-21 / P4-25** are lower-urgency, reachable only on corrupt, non-standard, or crafted inputs (SSE2-only IDCT i16-overflow, AVX2 unaffected; chroma-out-samples-luma reject; arithmetic same-slot DAC redefinition between scans). **P4-13 / P4-14 / P4-17** are C-ABI contract gaps to drive to closure as adoption pressure surfaces. Long-term backlog items live as P2-F (strided zero-copy refactor), P2-G (downstream lab uplift Qt+OpenCV), P2-H (OSS-Fuzz upstream PR + C-harness sanitizer), P2-I (remaining hard-case parity classes), and the per-ABI cdylib matrix / production packaging / no_std subset / security ops policy / cost-of-feature benchmarks — all tracked in the master plan at `/Users/yhkwon/.claude/plans/dreamy-moseying-swing.md`. None of these block the current T3 release gate: P4-13/14/17 document upstream contract-surface divergences for T4 framing, and P4-20/21/25 only trigger on corrupt or pathologically-crafted inputs.
