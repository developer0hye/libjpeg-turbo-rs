...
- **B9-2 Pillow / B9-3 ImageMagick**: link harnesses landed (`examples/pillow_smoke/`, `examples/imagemagick_smoke/`, `tests/capi_*_compat.rs`). Still `#[ignore]`d pending their own compat fixes.
...
- **libtiff end-to-end test wired** (`examples/libtiff_integration/` + `crates/libjpeg-turbo-rs-capi/tests/libtiff_integration.rs`): a C program opens a TIFF with `COMPRESSION_JPEG`, writes/reads strips via `TIFFWriteEncodedStrip` / `TIFFReadEncodedStrip`, with our cdylib staged as the JPEG provider via `DYLD_LIBRARY_PATH` / `LD_LIBRARY_PATH`. This exercises the real downstream consumer of `jpeg_write_raw_data` / `jpeg_read_raw_data` (PR #240/#241). **Currently `#[ignore]`d** due to a known gap: `jpeg_read_header` does not return `JPEG_HEADER_TABLES_ONLY` for the `JPEGTables` abbreviated datastream that libtiff prepends to each strip decode session, causing libtiff to report "Bogus JPEGTables field". The symbol-resolution (dlopen) path is already validated by `capi_stock_tool_link`; this test adds end-to-end round-trip validation and documents the remaining gap. Run with `--include-ignored` to reproduce the failure.
