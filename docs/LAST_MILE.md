...
Project is **replacement-ready** for the Rust-application + stock-tool drop-in story. System-library drop-in (Phase 2) is closed. Long-tail C-compatibility (Phase 3) has 2 OPEN items left (P3-5 PARTIAL, P3-6 OPEN).

**Live gate** (refresh whenever the inventory changes):

| Check | Result |
| --- | --- |
| `cargo test --workspace --release` | **Passes** product-path — 2144 tests, 0 product failures, 2 ignored (one pre-existing capi `imagemagick_roundtrips_through_our_cdylib` PSNR regression tracked under `docs/fix-arith-contradiction`). |
...
**Next up** (per Phase 3 Suggested Order below): **P3-5 pattern #4** (destination suspension via `empty_output_buffer` returning `FALSE`). After that, **P3-6** is the only remaining Phase 3 item (P2 priority — narrow consumer impact).
