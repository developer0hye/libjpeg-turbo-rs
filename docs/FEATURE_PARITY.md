...
- **Which upstream release each C oracle runs against (P4-130)**: apt ships 2.1.x on Ubuntu 24.04, which lacks `-lossless`, `-precision`, and SOF3 decode, so CI installs upstream's own `libjpeg-turbo-official_${VERSION}_${ARCH}.deb` under `/opt/libjpeg-turbo` instead. Since 2026-08-17 there are **two tool legs, not one**, and `docs/oracle_versions.tsv` is the machine-checked statement of what each one is:
  ...
  | --- | --- | --- |
  | `tool-baseline` | **3.1.4.1** (2026-03-27) | The behaviour-regression leg: `Integration Tests`, the `Full C Parity` exhaustive matrices on both architectures, the cross-arch and fuzz-smoke workflows, and `ci.yml`'s `test-corpus` (source build) and `test-cross-encode` (homebrew, still unpinned — P4-130's remainder names it). The release the expectations in this document were written against. Since 2026-08-18 the aarch64 `Full C Parity` leg builds it from source rather than taking `brew install jpeg-turbo`, which had no per-version formula and would have re-baselined the matrices silently the week homebrew moved. |
