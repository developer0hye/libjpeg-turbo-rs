...
- **CI oracles run against libjpeg-turbo 3.1.4.1 — one minor behind upstream (P4-130)**: apt ships 2.1.x on Ubuntu 24.04, which lacks `-lossless`, `-precision`, and SOF3 decode, so CI downloads the official `libjpeg-turbo-official_3.1.4.1_${ARCH}.deb` from upstream GitHub releases and prepends `/opt/libjpeg-turbo/bin` to `$PATH`. That reason still holds; the *version* has not been re-evaluated since **3.2.0 shipped 2026-06-30**. Every differential gate in this document therefore proves parity with **3.1.4.1**, and the 3.2 delta is **unmeasured** — read every ✅ backed by a C oracle as "matches 3.1.4.1", not "matches current upstream". 13 pin sites across five workflows; the dual-oracle matrix and the delta triage are P4-130 (#461). A single global bump is explicitly *not* the fix: it would silently re-baseline every existing expectation, making anything it papers over indistinguishable from a pass.

  **Upstream-release tracking policy (P4-130 criterion 4).** 3.2.0 sat unnoticed for two months because nothing was watching. Going forward:
  1. **Watch releases, not the calendar.** Subscribe the repo to `libjpeg-turbo/libjpeg-turbo` releases, or add a scheduled job that compares the newest upstream tag against the pin literal and opens an issue on mismatch. The check belongs in CI because a human reading release notes is what already failed.
  2. **A new upstream minor opens a triage item, not a bump.** Enumerate the release notes into deltas, and triage each to exactly one of: already at parity (naming the differential test that proves it), a new OPEN LAST_MILE entry, or a recorded non-goal. No delta left untriaged.
  ...
  4. **This document names the version each gate runs against**, so "parity" is never read as a claim about a release nobody tested.
