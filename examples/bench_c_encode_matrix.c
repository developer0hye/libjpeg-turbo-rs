...
static unsigned long compress_jpeg(const unsigned char *pixels, int w, int h,
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        ...
        /* cast away const — libjpeg API takes non-const but doesn't modify */
...
int main(void) {
    ...
    for (int c = 0; c < ncases; c++) {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            compress_jpeg(pixels, w, h, 75, cases[c].h_samp, cases[c].v_samp, &buf);
            free(buf);
        }
        ...
        printf("C_encode_%4dx%-4d_%-3s                              %4dx%-4d %10.1f us  (%d iters)\n",
