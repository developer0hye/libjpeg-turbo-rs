...
/// NEON FDCT from pre-transposed columns.
///
/// Same computation as `neon_fdct_core`, but accepts 8 pre-transposed column
/// vectors instead of loading from memory. Used by the fused extract+FDCT path
/// to avoid the intermediate `[i16; 64]` store between extract_block and FDCT.
///
/// # Safety
...
pub unsafe fn neon_fdct_from_cols(
    ...
    // ------ Pass 1: process rows (columns are already transposed) ------
