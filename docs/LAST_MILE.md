...
| P4-31 | [real-world and corpus CI gates can silently lose coverage](last_mile/phase4.md#p4-31-real-world-and-corpus-ci-gates-can-silently-lose-coverage--open) | 4 | OPEN | Rust panics/errors can become skips; nested real-world fixtures and extensionless JPEG seeds are omitted; textual summary parsing permits arbitrary skips and encode failures. Blocks the `v0.6.3` tag. |
| P4-33 | [checked-in `aspect_*` fuzz seeds drifted from the current encoder](last_mile/phase4.md#p4-33-checked-in-aspect_-fuzz-seeds-drifted-from-the-current-encoder--open) | 4 | OPEN | `cargo test` regenerates 14 tracked `aspect_wide_*` seeds with different bytes (830→922); reproduced on clean `main`, so an encoder change altered 8×64/64×8 4:2:0 output without seeds being re-committed. Found during issue #308 work. |
